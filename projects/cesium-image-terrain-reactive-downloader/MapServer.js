/*
 * deno run --allow-net --allow-read --allow-write --watch --unstable MapServer.js
 */
import{FileServer}from"../deno/FileServer/mod.js";
import{download}from"./mod.js";
let fileServer=new FileServer({
    port:8088,
    cors:true,
    onNotExistsFile:function(file){
        if(file.search("World_Imagery/MapServer/tile")>-1){//访问不存在的地图
            let[match,z,y,x]=file.match(/World_Imagery\/MapServer\/tile\/(\d*)\/(\d*)\/(\d*)/);
            if(z===undefined){return;}
            if(z>17){return;}
            download(`https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/${z}/${y}/${x}`,file);
        }else if(file.search(/1\/(\d*)\/(\d*)\/(\d*).terrain/)>-1){//访问不存在的地形
            let[match,z,y,x]=file.match(/1\/(\d*)\/(\d*)\/(\d*).terrain/);
            let headers=new Headers();
            headers.set("Authorization","Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..A6QkTvRgioGVBF1miilfCXI0F-7dkovxmj4WnPW3LCk")
            download(`https://assets.cesium.com/1/${z}/${y}/${x}.terrain?extensions=octvertexnormals-watermask-metadata&v=1.2.0`,file,{headers:headers})
        }else if(file.search("1/layer.json")>-1){//访问地形的索引文件
            let headers=new Headers();
            headers.set("Authorization","Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..A6QkTvRgioGVBF1miilfCXI0F-7dkovxmj4WnPW3LCk")
            download(`https://assets.cesium.com/1/layer.json`,file,{headers:headers})
        }
    }
});
fileServer.start();
