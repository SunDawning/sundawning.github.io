...
const importMeta = {
    url: "file:///C:/Users/sgs/AppData/Roaming/.emacs.d/site-lisp/deno/FileServer/mod.js",
    main: import.meta.main
};
function FileServer1(options) {
    let SELF = {
    };
    ...
    } else if (SELF.externalLoginServer) {
    ...
    SELF.start = async function() {
        ...
        for await (let request of server){
            let pathname = decodeURIComponent(new URL(request.url, "http://request.com").pathname);
            console.log(formatTime(), request.method, "访问", request.url, "=>", pathname);
            async function requestDirectory(request1, directory) {
                ...
                for await (let entry of Deno.readDir(directory)){
                    if (entry.isDirectory === true) {
                        ...
                        if ([
                            ...
                            ".jpg",
                            ".mp4", 
                        ].indexOf(extname2(entry.name).toLowerCase()) === -1) {
                            continue;
                        }
                        entries.push(entry);
                    }
            ...
            async function requestFile(request1, file, contentType) {
                if (file === undefined) {
                    file = new URL(request1.url, "http://request.com").pathname.substring(1);
                }
                if (existsSync(file) === false) {
                    console.log(formatTime(), "文件不存在", file);
                    if (SELF.onNotExistsFile) {
                        SELF.onNotExistsFile(file);
                    }
                    request1.respond({
                        status: 404
                    });
                    ...
                    if (contentType) {
                        if (contentType === "video/mp4") {
                            let rangeQuery = request1.headers.get("Range");
                    ...
                    try {
                        await request1.respond(response);
                    } catch (error) {
